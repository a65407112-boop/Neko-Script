...
local VERSION_FOLDER = CACHE_FOLDER .. "/3_29"
...
if not resolver then
	error(
		"[Caelus Neko 3.29] This executor has no "
...
if not folderOk then
	error(
		"[Caelus Neko 3.29] Could not create cache folder: "
...
if downloaded then
	...
	if not saved then
		warn("[Caelus Neko 3.29] Could not update cached hub.lua")
	end
else
	...
	if readfileFunction and fileExists(HUB_CACHE_PATH) then
		...
		if ok and type(cached) == "string" and #cached > 100 then
			hubSource = cached
			warn(
				"[Caelus Neko 3.29] GitHub unavailable; "
...
if not hubSource then
	error(
		"[Caelus Neko 3.29] Could not download hub.lua and no cache exists: "
			.. tostring(hubProblem),
		0
	)
end
...
if type(loadstring) ~= "function" then
	error("[Caelus Neko 3.29] loadstring() is unavailable.", 0)
end

local chunk, compileProblem = loadstring(hubSource, "=CaelusNekoHub3.29")
if not chunk then
	error(
		"[Caelus Neko 3.29] hub.lua compile failed: "
...
if not ok then
	error(
		"[Caelus Neko 3.29] hub.lua runtime failed: "
